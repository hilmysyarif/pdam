...
export function updateProfile(formData) {
  ...
  return dispatch => new Promise(async (resolve, reject) => {
    ...
    if (!firstName) return reject({ message: ErrorMessages.missingFirstName });
    if (!lastName) return reject({ message: ErrorMessages.missingLastName });
    if (!id_pelanggan) return reject({ message: ErrorMessages.missingIdPelanggan });
    if (!no_hp) return reject({ message: ErrorMessages.missingNoHp });
    if (!alamat) return reject({ message: ErrorMessages.missingAlamat });
    if (changeEmail) {
      if (!email) return reject({ message: ErrorMessages.missingEmail });
    }
    if (changePassword) {
    ...
    // Go to Firebase
    return FirebaseRef.child(`users/${UID}`).update({ firstName, lastName, id_pelanggan, alamat, no_hp })
      .then(async () => {
